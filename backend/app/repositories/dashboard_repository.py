...
def get_monthly_dashboard_totals(db: Session,current_user_id: int,month: int,year: int):
    ...
    expected_income=db.query(func.sum(IncomeModel.amount)).filter(IncomeModel.user_id==current_user_id,IncomeModel.status==IncomeStatus.EXPECTED,IncomeModel.received_date<next_month, IncomeModel.received_date >= first_day).scalar()
